  ...
  study: router({
    ...
    saveGeneratedDeck: protectedProcedure
      .input(z.object({
        ...
        cards: z.array(z.object({ front: z.string().min(8).max(500), back: z.string().min(8).max(1200), hint: z.string().min(3).max(300), mnemonic: z.string().min(3).max(300), questionType: z.enum(["multiple_choice", "identification"]), choices: z.array(z.string().min(1).max(300)).max(5), correctAnswer: z.string().min(1).max(500), aiDifficulty: z.enum(["easy", "medium", "hard"]), cognitiveSkill: z.enum(["remember", "understand", "apply", "analyze", "evaluate"]), questionRationale: z.string().min(12).max(240) })).min(3).max(30),
      }))
