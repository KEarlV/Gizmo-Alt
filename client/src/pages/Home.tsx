...
type CognitiveSkill = "remember" | "understand" | "apply" | "analyze" | "evaluate";

const difficultyLabels: Record<AIDifficulty, string> = { easy: "Warm-up", medium: "Build", hard: "Stretch" };
const skillLabels: Record<CognitiveSkill, string> = { remember: "Recall", understand: "Explain", apply: "Apply", analyze: "Analyze", evaluate: "Evaluate" };
...
function TopBar({ onFocusClick, onStartReview, onImport, isAuthenticated, user, isAdmin, onLogin, onSignUp, onLogout }: { onFocusClick: () => void; onStartReview: () => void; onImport: () => void; isAuthenticated: boolean; user: ProfileUser | null; isAdmin: boolean; onLogin: () => void; onSignUp: () => void; onLogout: () => void }) {
  return (
    <header className="top-bar">
      ...
      <div className="top-bar__actions">
        ...
        {isAuthenticated && user ? <ProfileDropdown user={user} isAdmin={isAdmin} onLogout={onLogout} /> : <><button className="top-bar__account top-bar__account--signup" onClick={onSignUp}>Create account</button><button className="top-bar__account" onClick={onLogin}>Sign in</button></>}
...
function AiImportModal({ onClose, onCreated }: { onClose: () => void; onCreated: (deck: Deck) => void }) {
  ...
  const [preview, setPreview] = useState<{ title: string; summary: string; mnemonic: string; cards: Array<{ front: string; back: string; hint: string; mnemonic: string; questionType: QuestionType; choices: string[]; correctAnswer: string; aiDifficulty: AIDifficulty; cognitiveSkill: CognitiveSkill; questionRationale: string }>; sourceFileKey: string; sourceFileName: string; sourceMimeType: string } | null>(null);
...
export default function Home() {
  ...
  return <div className="app-shell"><SideRail view={view} setView={setView} /><div className="app-main">{authError && <div className="auth-alert" role="status">Sign-in needs another try. <button onClick={() => startLogin()}>Reconnect</button></div>}{shareNotice && <div className="copy-toast" role="status"><Copy size={14} /> {shareNotice}</div>}<TopBar onFocusClick={() => setFocusInfoOpen((open) => !open)} onStartReview={() => startReview()} onImport={() => setAiImportOpen(true)} isAuthenticated={isAuthenticated} user={user} isAdmin={user?.role === "admin"} onLogin={() => startLogin()} onSignUp={() => startSignUp()} onLogout={() => { void logout(); }} />{focusInfoOpen && <div className="focus-popover"><div className="focus-popover__icon"><InfinityIcon size={20} /></div><div><strong>Deep study mode is on.</strong><span>No hearts to count, no cooldown to wait through. Keep going while the idea is alive.</span></div><button onClick={() => setFocusInfoOpen(false)} aria-label="Close focus info"><X size={15} /></button></div>}{view === "home" && <Dashboard decks={visibleDecks} onReview={startReview} onViewDecks={() => setView("decks")} onShowFocus={() => setFocusInfoOpen(true)} userName={user?.name} stats={accountDashboardQuery.data} />}{view === "decks" && <DecksView decks={visibleDecks} onReview={startReview} onCreate={() => setNewDeckOpen(true)} onDelete={deleteDeck} onRegenerate={regenerateDeck} onExport={exportDeck} onShare={shareDeck} />}{view === "insights" && <InsightsView />}{view === "library" && <LibraryView decks={visibleDecks} folders={foldersQuery.data ?? []} onReview={startReview} onMemorize={startMemorize} onDelete={deleteDeck} onRegenerate={regenerateDeck} onExport={exportDeck} onShare={shareDeck} onCreateFolder={createFolder} onAssignFolder={assignFolder} />}{view === "review" && selectedDeck && <ReviewView deck={selectedDeck} cardsOverride={importedDeckId === Number(selectedDeck.id) ? importedDeckQuery.data?.cards.map((card) => ({ prompt: card.front, answer: card.back, hint: card.hint ?? "Keep the core idea close.", questionType: card.questionType, choices: parseChoices(card.choices), correctAnswer: card.correctAnswer ?? card.back, aiDifficulty: card.aiDifficulty as AIDifficulty, cognitiveSkill: card.cognitiveSkill as CognitiveSkill, questionRationale: card.questionRationale ?? undefined, persistedId: card.id })) : undefined} onExit={() => setView("home")} />}{view === "memorize" && selectedDeck && <MemorizeView deck={selectedDeck} cards={importedDeckQuery.data?.cards.map((card) => ({ prompt: card.front, answer: card.back, hint: card.hint ?? "Keep the core idea close.", questionType: card.questionType, choices: parseChoices(card.choices), correctAnswer: card.correctAnswer ?? card.back, aiDifficulty: card.aiDifficulty as AIDifficulty, cognitiveSkill: card.cognitiveSkill as CognitiveSkill, questionRationale: card.questionRationale ?? undefined, persistedId: card.id })) ?? []} onExit={() => setView("home")} />}</div>{aiImportOpen && <AiImportModal onClose={() => setAiImportOpen(false)} onCreated={addImportedDeck} />}{newDeckOpen && <NewDeckModal onClose={() => setNewDeckOpen(false)} onCreate={createDeck} />}</div>;
}
